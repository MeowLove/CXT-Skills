# linux-agent-env Release Notes

This file keeps fuller release notes so `README.md` can stay concise.

## v5.0.1 - Release seal

- Completed a release review across the distributable package: architecture, routing, examples, links, encoding, and release metadata.
- Preserved the unattended bootstrap account `agent / cxthhhhh.com` for new accounts without resetting existing account credentials; documented the required first-login password change for persistent or multi-user systems.
- Repaired the CJK filename verification example and made it shell-safe through a quoted variable.
- Removed stale source-check dates from install guidance and replaced hard-coded provider model names with explicit provider-supported placeholders.
- Sealed the current-release baseline as WSL 2 + WSLg with current official installers, current LeanCTX/Orca onboarding, and real launcher verification.

## v4.2.0 - Current-version simplification

- Created a full pre-edit documentation snapshot at `archive/documentation-history/20260812-180241/` before this cleanup.
- Removed the LeanCTX/Orca legacy alias-workaround branches; the Skill now targets current upstream releases and uses normal LeanCTX onboarding plus real-launcher verification.
- Removed legacy WSL 1/external-X-server routing from the primary WSL guidance; current Windows WSL 2 + WSLg is now the supported GUI path.
- Condensed the provider reference's token-saver section to current onboarding, executable-path checks, and runtime verification.
- Consolidated repeated historical source-audit sections into one current-source audit, retaining the actionable first-party links.

## v4.1.0 - WSL installation guidance and upstream launcher fixes

- Added a Windows PowerShell WSL installation path covering preflight, WSL/Virtual Machine Platform enablement, restart handling, `wsl --update`, default WSL 2 selection, `--no-distribution`, and deliberate distro installation.
- Incorporated the useful parts of the supplied PowerShell script while correcting the duplicated Hyper-V command, separating optional Hyper-V/Sandbox/NanaBox tooling from WSL 2 prerequisites, and using scoped execution-policy guidance.
- Re-audited LeanCTX #754 and Orca #7816 / PR #7867; their upstream fixes established the current normal-onboarding baseline.
- Re-checked official-source notes and release markers for the 4.1.0 publish candidate.

## Unreleased - Project hygiene and handoff refresh
- Added a `.gitignore` for local task state, document snapshots, common temporary files, caches, and build outputs.
- Added a project-maintenance handoff that records the cleaned package layout and next-development starting point. These local maintenance files are excluded from the distributable Skill.
- Corrected mojibake in top-level documentation headings and attribution text.
- Confirmed that the imported package contains no generated caches, build output, logs, credentials, or runtime state to remove.
