...
Official docs checked 2026-07-08: `https://docs.anthropic.com/en/docs/claude-code/getting-started`. Anthropic currently documents **global npm installation as the standard install path** and explicitly warns not to use `sudo npm install -g`. The docs also list a native binary installer as **Alpha**: `claude install` from an existing install, or `curl -fsSL claude.ai/install.sh | bash` for a fresh install.

Use this decision order:

1. If the user asks for the strict current official-standard install, use npm under the normal user.
2. If the user wants no Node/npm global dependency, wants a local/native binary path, or npm permissions are a problem, use the official native Alpha installer after telling the user it is marked Alpha upstream.
3. Use apt/dnf/apk repository installation only when the user explicitly chooses a managed package path or current official docs make that path selected.

```bash
# Official standard path; requires Node.js 18+ and npm. Do not use sudo npm install -g.
npm install -g @anthropic-ai/claude-code

# Official native binary installer, currently marked Alpha by upstream.
...
1. If the user wants no Node/npm dependency or asks for the official standalone/native path, use the standalone installer from the OpenAI GitHub README.
2. If the user already has a managed current Node.js/npm toolchain, or the standalone installer is blocked by network policy, use npm.
3. Do not mix standalone and npm-managed Codex without first checking `type -a codex`, `readlink -f "$(command -v codex)"`, and the update path.

```bash
# Standalone installer for macOS/Linux/WSL from OpenAI/openai-codex README
curl -fsSL https://chatgpt.com/codex/install.sh | sh

# npm alternative from OpenAI docs/help; requires Node.js/npm
