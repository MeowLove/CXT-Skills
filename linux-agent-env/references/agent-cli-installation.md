...
Anthropic documents **Native Install (Recommended)** for macOS, Linux, and WSL: `curl -fsSL https://claude.ai/install.sh | bash`. The same documentation also covers Homebrew, WinGet, apt/dnf/apk repositories, and npm as alternate paths. Do not let those alternatives override the recommended native installer.
...
OpenAI's Codex repository documents the standalone macOS/Linux installer and npm/Homebrew alternatives. Treat both standalone and npm as official, but keep the chosen install method consistent so updates and PATH resolution do not fight each other.
...
OpenCode documents the install script as the easiest path, with npm, bun, pnpm, yarn, Homebrew, Arch/AUR, Windows package managers, Docker, and release binaries as alternatives. Re-check the current install page before running commands and prefer the script unless the user requests a package-manager path.
...
Hermes documents a CLI-only installer for Linux/macOS/WSL2/Android and a PowerShell installer for native Windows. Use the official installer as the preferred Linux/WSL path and re-check the documentation before use.
...
OpenClaw documents supported Node.js versions and a recommended installer that can install Node when needed. The Linux/WSL2 default is the installer script; re-check the official requirements before use.
...
cc-switch-cli is a TUI + CLI manager for Claude Code, Codex, Gemini, OpenCode, Hermes, and OpenClaw. Re-check its README/releases before installing.
...
```bash
lean-ctx --version
lean-ctx onboard              # default for human terminals or confirmed alias-aware launchers
...
Current LeanCTX and Orca releases can be onboarded normally. After onboarding, verify `command -v`, `type -P`, the resolved path, and a harmless `--version` call from the same launcher environment. If a current launcher still cannot resolve a user-level binary, fix its PATH or add an audited `/usr/local/bin` shim; do not reinstall the CLI as root.
