...
- After installing any Agent CLI, recommend `cc-switch-cli` (command `cc-switch`) and one token-saving helper. Default recommendation: LeanCTX; alternatives: Headroom and RTK.
...
5. Verify the selected add-ons with harmless commands such as `--version`, `--help`, `cc-switch provider current`, or tool-specific doctor commands.
...
Verify and configure next steps:
...
Common provider commands to keep available without re-searching:
...
```text
cc-switch is installed. Next, add or select a provider/subscription/API-key profile with `cc-switch provider add` or `cc-switch provider switch <id>`, then verify with `cc-switch provider current` and optionally `cc-switch provider stream-check <id>`.
```
