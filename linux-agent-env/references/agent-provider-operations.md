...
3. **System-visible shim when needed**: if a current GUI/IDE/service launcher cannot see the user PATH, link `/usr/local/bin/<tool>` to the verified user-level entrypoint. Treat this as an optional launcher integration, not the primary install location.
...
## Token Saver and Launcher Verification

Use this section when LeanCTX, Headroom, or RTK is installed with Orca, a GUI manager, IDE plugin, SSH launcher, Windows-to-WSL wrapper, CI/service, or a non-interactive shell.

Keep executable discovery and runtime injection separate. Current LeanCTX and Orca releases support normal onboarding:
...
Verify from the same environment that launches the Agent:
...
for t in claude codex cc-switch lean-ctx headroom rtk; do
  echo "== $t =="
  type -P "$t" 2>/dev/null || command -v "$t" 2>/dev/null || true
  "$t" --version 2>/dev/null || "$t" version 2>/dev/null || true
done
```

If a current launcher finds the CLI but does not show token savings, inspect the selected tool's status and the launcher's documented environment/config; do not stack token savers while diagnosing.
