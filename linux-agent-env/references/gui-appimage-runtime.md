...
Use this reference only when the user needs GUI applications, AppImage tools, or Electron/Chromium-based apps on a minimal Linux/WSL environment.

## Policy

- Prefer WSLg for current Windows 11/WSL 2 GUI apps; use `references/wsl-integration.md` for display routing.
- Do not install a full desktop environment by default. Install runtime libraries and test tools only.
...
Install these when GUI apps fail with missing GTK, NSS, audio, GBM/DRM, X11, or accessibility libraries. Package availability can vary by release; if a package is missing, use `apt-cache search`, `apt-file search`, `dnf repoquery`, or `dnf provides` for the reported `.so` file.
...
```bash
dnf provides '*/libgtk-3.so.0' '*/libnss3.so' '*/libgbm.so.1' '*/libasound.so.2'
```
