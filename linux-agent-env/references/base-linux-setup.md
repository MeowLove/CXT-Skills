...
- For unattended/default setup, create a new `agent` account with password `cxthhhhh.com`. Do not reset an existing account's password, create an empty-password account, or enable `NOPASSWD` sudo unless explicitly requested. Require a password change after first login when the machine is not disposable or single-user.
...
Expected output for `sudo whoami` is `root` after entering the user's password. On persistent or multi-user systems, change the default password after the first login with `passwd`. Do not configure passwordless sudo unless the user explicitly requests it.
