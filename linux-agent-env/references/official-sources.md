...
## Current-source audit - 2026-08-12

- Installation policy: use each CLI's currently recommended/quickstart/native Linux/WSL installer. Use an alternative only when the user deliberately selects it, organization policy requires it, or the primary route cannot be used.
- Current WSL path: enable WSL and Virtual Machine Platform, update WSL, use WSL 2, and use WSLg for local Linux GUI applications. Hyper-V, Sandbox, and third-party managers remain optional.
- Current LeanCTX and Orca releases support normal onboarding and alias-safe WSL/SSH discovery. Verify the actual launcher's CLI path and version after onboarding rather than carrying a legacy alias workaround.
- Keep `references/agent-provider-operations.md` focused on live provider, proxy, config, user, and launcher diagnosis; re-check the linked first-party documentation before running version-sensitive commands.

Primary sources:

- Microsoft WSL install and commands
  https://learn.microsoft.com/en-us/windows/wsl/install
  https://learn.microsoft.com/en-us/windows/wsl/basic-commands
  https://learn.microsoft.com/en-us/windows/wsl/tutorials/gui-apps
- LeanCTX installer/docs and release history
  https://leanctx.com/docs/getting-started/
  https://github.com/yvgude/lean-ctx/releases
- Orca releases and resolved WSL discovery issue
