...
## Token saver / launcher compatibility audit - 2026-07-09

- GNU Bash manual: non-interactive Bash expands `BASH_ENV` and uses the expanded value as a startup file; this supports the Skill rule to avoid global `BASH_ENV` and prefer per-launcher injection when token tools need shell hooks.
  https://www.gnu.org/software/bash/manual/bash.html
- GNU Bash manual: Bash's `type` builtin reports command resolution; `type -P` forces a PATH lookup and can bypass aliases/functions when launchers need an executable path.
  https://www.gnu.org/software/bash/manual/bash.html#index-type
- LeanCTX official docs/site: preserve official install/onboarding flow (`curl -fsSL https://leanctx.com/install.sh | sh`, then `lean-ctx onboard`/doctor/status). The Skill adds a field-tested fallback for launchers that cannot resolve canonical aliases.
  https://leanctx.com/docs/getting-started/
- User-provided field issue links for LeanCTX/Orca launcher compatibility: these track the alias-based discovery problem and related upstream fixes reported during real deployments. Treat the URLs as field references to re-check, not as a substitute for current official README/help output. Keep the compatibility decision tree for older/unknown launchers even when current upstream versions are fixed.
  https://github.com/yvgude/lean-ctx/issues/754
  https://github.com/stablyai/orca/issues/7816
  https://github.com/stablyai/orca/pull/7867
- Orca official README: Orca can run CLI agents and is distributed for Linux/macOS/Windows; verify CLIs from Orca's real launch environment instead of assuming the interactive shell PATH applies.
  https://github.com/stablyai/orca

## v4.0.1 release audit - 2026-07-09

Primary-source review outcome: keep the v3.2.1 installer precedence model for v4.0.1. For Linux/WSL, prefer each upstream's recommended/native installer script first, then use npm/pipx/apt/dnf/Homebrew/manual archives only by explicit user choice, organizational policy, offline/pinned-artifact need, or installer failure.

- Claude Code: preserve Native Install (Recommended) first for macOS/Linux/WSL; npm and OS package managers remain alternatives, not the default.
  https://code.claude.com/docs/en/getting-started
- OpenAI Codex CLI: preserve the standalone installer from OpenAI/openai-codex for Linux/macOS as the default Skill path; npm remains official but is not selected merely because Node.js exists.
  https://github.com/openai/codex
  https://help.openai.com/en/articles/11096431-openai-codex-cli-getting-started
- cc-switch-cli: preserve the latest-release `install.sh` quick install and app-scoped provider commands such as `cc-switch --app codex provider list`.
  https://github.com/SaladDay/cc-switch-cli
- OpenCode, Hermes, and OpenClaw: preserve their documented Linux/WSL installer scripts as default paths and keep package-manager alternatives as deliberate fallback/user-choice paths.
  https://opencode.ai/docs/
  https://hermes-agent.nousresearch.com/docs/getting-started/installation
  https://docs.openclaw.ai/install
- GUI managers: keep release-asset-first guidance for Orca/TOKENICODE, web/server packaging guidance for Polaris, and Node 22+ guidance for CloudCLI/claudecodeui.
  https://github.com/stablyai/orca
  https://github.com/misxzaiz/Polaris
  https://github.com/yiliqi78/TOKENICODE
  https://github.com/siteboon/claudecodeui
- WSL/GUI: keep WSLg as the default modern Windows 11 WSL 2 GUI route and external X Server as an intentional fallback; verify `DISPLAY`, `WAYLAND_DISPLAY`, and `/mnt/wslg` from the target distro/session.
  https://learn.microsoft.com/en-us/windows/wsl/tutorials/gui-apps
  https://github.com/microsoft/wslg

## v4.0.2 reference consolidation and field-origin audit - 2026-07-09

- Reference layout review: no files were merged for v4.0.2. The current nine reference files map to progressive-disclosure layers and prevent Agents from loading provider/debug/source-audit material during simple locale, WSL, or toolchain tasks.
- CXT/MeowLove v4 provenance: v4 was driven by real deployment failures and issue discussions with multiple open-source Agent/tool projects around external GUI/IDE/Agent compatibility. The resulting rules are operational guardrails, not just documentation style: verify the real launcher environment, prefer user-level stable CLI paths plus audited shims, and separate executable discovery from LeanCTX/Headroom/RTK runtime injection.
- Official-source result remains unchanged from v4.0.1: recommended/native installer scripts stay first for Linux/WSL Agent CLIs; npm/pipx/apt/dnf/Homebrew/manual archives remain explicit fallbacks.
