...
Use this reference when the user wants a graphical/web control layer for Agent CLIs after the base Linux/WSL environment and at least one Agent CLI are installed. These tools change quickly; official sources were re-checked on 2026-07-09, but still re-check upstream release assets and README before downloading or executing installers.
...
- Verify required CLIs before launching a GUI from the same user/environment that will run it: `claude --version`, `codex --version`, `opencode --version`, `hermes --version`, `openclaw --version`, or the tool-specific command. If the GUI cannot see user-installed CLIs under `~/.local/bin`, load `references/agent-provider-operations.md` and create audited `/usr/local/bin` shims instead of reinstalling as root.
...
|---|---|---|---|---|
| Orca (`stablyai/orca`) | Full desktop orchestrator for multiple agents and worktrees | Desktop app, release assets | Any CLI agent; upstream lists Claude Code, Codex, Gemini, Hermes Agent, OpenCode, Cursor, Grok, Pi, and more; mobile companion | Best for local desktop/WSLg users who want parallel worktrees and terminal splits. |
...
For web/npm managers, load `references/developer-toolchain.md` and make sure Node.js is current enough. CloudCLI currently documents Node.js v22+ for `npx @cloudcli-ai/cloudcli`. Polaris source builds require Node.js >=18 and Rust >=1.70.
...
Use Orca when the user wants a desktop orchestrator for parallel Agent worktrees, terminals, source control, and mobile companion monitoring. Upstream says Orca can run any CLI agent and is available for macOS, Windows, and Linux. Prefer official desktop release assets or the official download page:
...
- Arch Linux: upstream documents AUR packages such as `stably-orca-bin` and `stably-orca-git`
...
- For WSL, use WSLg and `references/gui-appimage-runtime.md` before running the AppImage. Before blaming Orca itself, check whether the Orca-launched terminal can resolve `claude`, `codex`, and `cc-switch`; if not, add `/home/agent/.local/bin` to the launcher PATH or create audited `/usr/local/bin` shims via `references/agent-provider-operations.md`.
- If LeanCTX or another token saver is installed and Orca cannot find Claude/Codex, check whether `command -v claude` or `command -v codex` returns alias/function text. Use the **Token Saver and External Launcher Compatibility** section in `references/agent-provider-operations.md`: prefer normal onboarding for alias-aware/fixed Orca versions, but use `lean-ctx onboard --no-agent-aliases` plus launcher-specific env or non-conflicting aliases for older/unknown launchers.
...
Use Polaris when the user wants a GUI for OpenAI Codex CLI and Claude Code CLI, especially when a WSL/server browser-accessible web mode is useful. Upstream README lists Node.js >=18, Rust >=1.70, Codex CLI when using the Codex engine, and Claude Code CLI when using the Claude engine.
...
Use this when no matching release asset exists or the user wants current source. Upstream README documents `pnpm run package:web` as the independent web service package for Linux servers/WSL/headless use:
...
Use TOKENICODE when the user wants a Claude Code-focused desktop client with session management, file explorer/editor, permission workflow, MCP/skill management, and provider/API preset UI. Upstream README describes it as a desktop client for Claude Code, with Claude Code CLI as a prerequisite that can be auto-detected/installed during first launch.

Prefer official release assets. README documents macOS `.dmg`, Windows `.msi`/`.exe`, and Linux `.AppImage`/`.deb`/`.rpm`; Linux requires WebKit2GTK. However, the newest GitHub release may temporarily contain only a subset of platforms. If the latest release lacks Linux assets, inspect recent stable releases and choose the newest matching Linux asset instead of building blindly.
