...
V4 field origin: CXT/MeowLove upgraded this Skill after real deployments and upstream issue discussions with multiple open-source Agent/tool projects exposed external GUI/IDE/Agent launcher incompatibilities. Preserve the v4 model: verify the real launcher environment, keep canonical CLI discovery separate from token-tool runtime injection, and prefer official-native installers before fallback package managers.
...
11. **Token/context helpers**: recommend one token saver by default. Default: LeanCTX. Alternatives: Headroom or RTK. Install only one unless the user wants comparison. Separate command discovery from token-tool runtime injection; aliases/functions/wrappers must not hide canonical `claude`/`codex` executables from launchers unless the launcher is confirmed alias-aware.
...
- Prefer stable user-level command paths such as `~/.local/bin/<tool>` and verify with `type -a`, `type -P`/`type -p`, `command -v`, `readlink -f`, and `--version`. If Orca/GUI/IDE/service launchers cannot resolve those paths, create audited `/usr/local/bin/<tool>` shims that point back to `/home/agent/.local/bin/<tool>` rather than reinstalling the CLI as root. If a token saver creates aliases/functions, keep discovery pointed at the real binary and restore runtime injection separately.
