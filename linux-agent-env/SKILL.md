...
description: Prepare Linux/WSL agent workstations with CJK fonts and filenames, English UTF-8 locale, WSLg/X Server routing, GUI/AppImage runtime libraries, developer toolchains, Git/SSH setup, stable Agent CLI paths, cc-switch provider checks, optional GUI management platforms, token-saver guidance, session handoff notes, timezone/RTC handling, safe Windows-to-WSL execution, least-privilege users, and WSL default-user setup. Use for Debian/Ubuntu/WSL and Fedora/RHEL setup, LANG=en_US.UTF-8, WSL GUI troubleshooting, Claude/Codex/OpenCode/Hermes/OpenClaw/cc-switch-cli/LeanCTX/Headroom/RTK/Orca/Polaris/TOKENICODE/CloudCLI planning, and root/sudo admin.
metadata:
  version: 2.5.1
...
2. Detect distro and environment:
   ...
   - If the user gives a target WSL distribution name, check whether it already exists before creating/importing/installing a distro. Use `references/wsl-integration.md` for the WSL distribution preflight, including normalized `wsl.exe` output and `HKCU:\Software\Microsoft\Windows\CurrentVersion\Lxss` registry checks.
...
13. After any Agent CLI install, recommend `cc-switch-cli`; after installing `cc-switch`, remind the user to add/select a provider or subscription with the app-scoped form `cc-switch --app <app> provider add` / `cc-switch --app <app> provider switch <id>`. Also recommend one token saver (LeanCTX by default; Headroom/RTK alternatives).
...
The base Linux/WSL locale, CJK font, timezone, RTC, and user commands are intentionally kept in this `SKILL.md` to avoid command drift. Use `references/session-continuity.md` for context-safe handoff files during long-running work. Use `references/wsl-integration.md` for Windows-to-WSL execution, WSL default users, WSLg/X Server routing, and WSL GUI troubleshooting. Use `references/gui-appimage-runtime.md` for minimal GUI/AppImage/Electron/Tauri runtime dependencies. Use `references/developer-toolchain.md` for optional Node.js/Python/terminal-tool/Git-SSH/workspace enhancements. Use `references/agent-cli-installation.md` for optional Agent CLI, provider-switching, and token-saver installation planning. Use `references/agent-cli-operational-checklist.md` after Agent CLI installs or when commands/providers fail in non-interactive shells. Use `references/agent-gui-management.md` for optional Orca/Polaris/TOKENICODE/CloudCLI graphical or browser management platforms. Use `references/official-sources.md` only when auditing or updating the skill against upstream docs.


## Packaging Note

For GitHub distribution, the minimal portable package is the `linux-agent-env/` directory containing `SKILL.md` plus the `references/` directory. The `agents/` directory is Codex UI metadata and may be omitted when the target Agent only needs the skill instructions. The former duplicate distro command reference has been merged into `SKILL.md`; do not add a second distro command file unless the main commands become too large. Do not omit current `references/`, including `session-continuity.md`, `developer-toolchain.md`, `agent-cli-operational-checklist.md`, and `agent-gui-management.md`, unless you first merge the needed reference content into `SKILL.md`.

For GitHub installs, use the repository tree URL `https://github.com/MeowLove/CXT-Skills/tree/main/linux-agent-env`, or installer parameters `--repo MeowLove/CXT-Skills --path linux-agent-env --ref main`. The browser URL without `/tree/<branch>/`, such as `https://github.com/MeowLove/CXT-Skills/linux-agent-env`, is not a valid GitHub web directory URL even though some installers may parse it as a repo-plus-subpath shorthand.

The `metadata.version` value is a human/Agent release marker only. Codex skill selection reads `name` and `description`; update buttons and remote/source comparison are client or installer behavior. For reproducible releases, prefer Git tags such as `v2.5.1` and install with `--ref v2.5.1`; use `main` only when the user wants the latest moving version.

## Windows-to-WSL Command Execution from Agents

Use `references/wsl-integration.md` when an Agent controls WSL from Windows PowerShell/CMD, when a named WSL distribution may already exist, or when sandboxed Agent visibility differs from the user's interactive Windows session.
...
Before creating, importing, or installing a WSL distribution with a requested name, prove the name is not already present. If `wsl --install`, `wsl --import`, or a Store/installer flow reports that the distribution already exists or the name is reserved, stop creation attempts immediately and switch to discovery/reuse. Repeated same-name creation retries usually hide an existing distro, a different Windows-user view, or unreadable/garbled command output.

Use the user's interactive Windows account as the authority whenever possible. A sandboxed/service Agent may see a different WSL registration view than the user's PowerShell window. If Agent-side `wsl -l -v` says no distro exists but Windows reports a name collision, ask to run the preflight from the user's normal PowerShell, or with the required external/elevated execution path.

Minimal preflight from Windows PowerShell:
...
'== normalized wsl names =='
$WslNames
'== registry distros =='
$RegDistros | Sort-Object DistributionName | Format-Table -AutoSize

if (($WslNames -contains $Target) -or ($RegDistros.DistributionName -contains $Target)) {
  "FOUND: $Target already exists; do not create/import another distro with this name."
  wsl.exe --distribution $Target -- whoami
} else {
  "NOT FOUND from this Windows user/session: $Target"
}
```

If output is garbled or empty, do not assume absence. Save structured output instead:

```powershell
$RegDistros | ConvertTo-Json -Depth 4
$WslNames | ConvertTo-Json
```
...
Use `references/session-continuity.md` when a setup or project task may outgrow the model context, needs a durable handoff, or involves multiple agents. Use `references/agent-cli-operational-checklist.md` after installing Agent CLIs, when external Agents cannot find commands, when PATH/symlink resolution is suspicious, or when cc-switch/Codex provider behavior is wrong. Use `references/agent-cli-installation.md` when the user asks to install Claude Code, OpenAI Codex CLI, OpenCode, Hermes, OpenClaw, CC-Switch, `cc-switch-cli`, or similar Agent tooling. Use `references/agent-gui-management.md` when the user asks for GUI/web management platforms such as Orca, Polaris, TOKENICODE, CloudCLI, or claudecodeui.
...
- After a requested Agent CLI is installed, load `references/agent-cli-operational-checklist.md` for final path/PATH/symlink/version checks when the environment is non-interactive, WSL, multi-user, or externally controlled. Suggest `cc-switch-cli` (command `cc-switch`). After `cc-switch` is installed, remind the user to add/select a provider, subscription, account, or API-key profile before use. Also ask whether the user wants a GUI management platform and one token-saving helper. Default token-saver suggestion: LeanCTX; alternatives: Headroom, RTK.
...
readlink -f ~/.local/bin/claude ~/.local/bin/codex ~/.local/bin/cc-switch 2>/dev/null || true
command -v cc-switch >/dev/null 2>&1 && cc-switch --app claude provider current || true
command -v cc-switch >/dev/null 2>&1 && cc-switch --app claude provider list || true
command -v cc-switch >/dev/null 2>&1 && cc-switch --app codex provider current || true
command -v cc-switch >/dev/null 2>&1 && cc-switch --app codex provider list || true
...
- A sandboxed or service-run Agent process may not see the same WSL distributions as the interactive Windows user. If `wsl -l -v` differs between Codex and the user's PowerShell, or if a creation/import command reports the name already exists while the Agent list appears empty/garbled, stop retrying creation and rerun the WSL preflight from the user's Windows session or with the required approval outside the sandbox.
...
- Before creating or importing a WSL distro by name, check both normalized `wsl.exe --list --quiet` output and the `HKCU:\Software\Microsoft\Windows\CurrentVersion\Lxss` registry `DistributionName` values. Treat garbled `wsl` output, invisible names, and "already exists" errors as a discovery problem, not as permission to retry creation.
...
- If a `cc-switch` provider switch unexpectedly reintroduces old auth, MCP, hooks, or model settings, load `references/agent-cli-operational-checklist.md`; avoid implicit common-config merges for critical Codex/Claude providers and use explicit raw config where deterministic auth fields are required.
- For `cc-switch` automation and documentation examples, prefer the current upstream/global app selector form: `cc-switch --app <app> provider ...`. Use non-app-scoped `cc-switch provider ...` only for explicitly global/all-app inspection or interactive TUI workflows.
...
- If installed Agent CLI commands are missing, resolve to root/npm/apt versions, or fail only in non-interactive shells, load `references/agent-cli-operational-checklist.md`; verify user, HOME, PATH, `type -a`, `readlink -f`, symlinks, and versions.
- If `cc-switch` is installed but Agent calls still fail or no provider is active, load `references/agent-cli-operational-checklist.md`; run per-app `cc-switch --app <app> provider list/current`, add or switch a provider/subscription/API-key profile using `cc-switch --app <app> provider ...`, inspect live config files, and verify any local proxy process/port.
- If WSL creation/import repeatedly fails with a same-name error, or if `wsl -l` output is mojibake/empty in the Agent sandbox, do not retry creation. Load `references/wsl-integration.md`, run the WSL distribution preflight from the user's interactive Windows session, and reuse the existing distro if found.
