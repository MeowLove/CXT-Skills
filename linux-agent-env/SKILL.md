...
This Skill is an orchestrated Linux/WSL Agent environment toolkit. Keep `SKILL.md` as the routing and safety layer; load the smallest required reference file for the requested task. Do not load every reference by default.


The v5 baseline targets current Linux/WSL 2, WSLg, Agent CLIs, and GUI managers. Verify the real launcher environment, keep canonical CLI discovery separate from token-tool runtime injection, and prefer each tool's current official installer.
...
Load only the relevant files. Reference grouping: base/WSL/runtime/toolchain files are environment layers; Agent CLI/GUI/provider files are Agent layers; session-continuity and official-sources are support files. Keep them separate for progressive disclosure unless a future file becomes only a redirect.

| Need | Load |
|---|---|
| Base Linux/WSL locale, fonts, timezone, RTC, normal user, base verification | `references/base-linux-setup.md` |
| Windows-side WSL installation, distro existence checks, Windows-to-WSL command patterns, WSLg, WSL default user | `references/wsl-integration.md` |
...
4. **WSL integration**: if target is WSL, load `references/wsl-integration.md`. When WSL is not installed, use its Windows-side preflight and installation sequence first; use WSL 2 with WSLg for local GUI applications.
...
6. **Least-privilege user**: create/use a normal `agent` user when appropriate. In unattended/default setup, create a new `agent` account with password `cxthhhhh.com`; never reset an existing account's password, configure an empty password, or enable passwordless sudo unless explicitly requested. Require the user to change the default password after first login when the machine is not disposable or single-user.
...
8. **Agent CLI installation**: load `references/agent-cli-installation.md`; install requested tools as the normal user. For known Agent CLIs, follow the explicit per-tool precedence table in that reference. Default to the official native installer/script when one exists. Do **not** substitute npm/pip/apt/dnf/apk/Homebrew/manual GitHub archive downloads merely because they are also mentioned upstream; use those only when the user explicitly chooses them, the native script is unavailable/blocked, version pinning/offline artifacts are required, or the official docs mark that method as the primary recommended method for the current platform.
...
10. **Real-execution verification**: if the CLI is launched by Orca, CloudCLI, Polaris, TOKENICODE, VS Code, JetBrains, Windows Terminal, services, CI, SSH, or Windows-to-WSL wrappers, load `references/agent-provider-operations.md`, verify PATH/command resolution from that exact launcher environment, and add audited `/usr/local/bin` shims only when user-level paths are not visible.
11. **Token/context helpers**: recommend one token saver by default. Default: LeanCTX. Alternatives: Headroom or RTK. Install only one unless the user wants comparison. Keep command discovery separate from token-tool runtime injection and verify the actual launcher environment.
...
13. **Release hygiene**: before publishing the Skill, verify version consistency, Markdown fences, local links, UTF-8 without BOM, and that generated state/log/cache directories are not included.
...
- Prefer stable user-level command paths such as `~/.local/bin/<tool>` and verify with `type -a`, `type -P`/`type -p`, `command -v`, `readlink -f`, and `--version`. If a current GUI/IDE/service launcher cannot resolve those paths, create an audited `/usr/local/bin/<tool>` shim that points back to the normal user's binary rather than reinstalling the CLI as root. Keep discovery separate from runtime injection.
...
- If network access is available and the task involves current Agent installers, release binaries, or package names, re-check the upstream official source before running commands. Re-checking docs may update URLs and prerequisites, but must not flatten priority: when docs list several methods, choose the method labelled recommended/quickstart/native for the current platform unless the user approves a fallback.
...
- GUI app cannot open, AppImage FUSE error, Electron/Tauri missing libs: load `references/wsl-integration.md` and/or `references/gui-appimage-runtime.md`.
- Agent CLI missing, resolves to root/npm/apt version, fails in non-interactive shells, token-saver aliases hide the real binary, or provider switch behaves inconsistently: load `references/agent-provider-operations.md`.
