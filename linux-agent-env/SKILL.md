...
The `metadata.version` value is a human/Agent release marker only. Codex skill selection reads `name` and `description`; update buttons and remote/source comparison are client or installer behavior. For reproducible releases, prefer Git tags such as `v2.3.1` and install with `--ref v2.3.1`; use `main` only when the user wants the latest moving version.
