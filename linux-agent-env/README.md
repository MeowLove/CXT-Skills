...
Current release marker: `2.5.1`

This repository release is intended as `v2.5.1`. Use the matching Git tag for stable installs after tagging the repository.
...
- Aligns the release marker and tagged install examples to `v2.5.1`.
- Keeps `cc-switch-cli` examples app-scoped by default with `cc-switch --app <app> provider ...`.
- Includes WSL distribution existence preflight guidance to prevent repeated same-name create/import attempts when Agent sandbox output is empty, garbled, or inconsistent with the user's interactive PowerShell session.
...
```bash
install-skill-from-github.py --repo MeowLove/CXT-Skills --path linux-agent-env --ref v2.5.1
```

or use the tagged tree URL:

```text
https://github.com/MeowLove/CXT-Skills/tree/v2.5.1/linux-agent-env
```
...
3. Create Git tags such as `v2.5.1`, `v2.5.2`, `v2.6.0` for reproducible installs.
