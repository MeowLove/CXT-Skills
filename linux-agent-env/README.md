...
Current release marker: `5.0.1`

`linux-agent-env` is a portable Skill for preparing current Linux and WSL Agent workstations with automation-ready account bootstrap that preserves existing credentials.

It covers reliable CJK/Chinese filename and GUI font display, English UTF-8 default locale, WSL 2 + WSLg, least-privilege users, developer tooling, Agent CLI installation, GUI management platforms, provider switching, provider mismatch/proxy/protocol diagnostics, token-saving helpers, and resumable handoff notes for long-running Agent work.
...
- WSL-specific guidance for Windows-side installation, optional feature enablement, distro existence preflight, WSLg, default users, and Windows-to-WSL command quoting
...
- Provider operations for stable user paths, optional `/usr/local/bin` shims for current GUI/IDE launchers, PATH, symlinks, non-interactive shells, cc-switch provider lifecycle, live config verification, proxy/protocol troubleshooting, and launcher mismatch debugging
- GUI management platform guidance for Orca, Polaris, TOKENICODE, and CloudCLI/claudecodeui
- Token-saving helper guidance for LeanCTX, Headroom, and RTK, with launcher verification and hook/proxy isolation
...
Copy the complete `linux-agent-env/` directory into the target Agent's skills directory. Keep `SKILL.md`, `README.md`, `CHANGELOG.md`, `.gitignore`, and the full `references/` directory together. Local maintenance folders such as `handoff/` and `archive/` are intentionally excluded and are not part of the distributable Skill.
...
There is no universal Skill update marker that every Agent client honors. Keep `metadata.version` in `SKILL.md`, this README marker, Git tags, and `CHANGELOG.md` synchronized. For reproducible installs, install from a tag such as `v5.0.1`; for latest installs, use `main`.
...
Before publishing, include only the distributable Skill files and avoid committing generated state such as `.agent-state/`, `handoff/`, `archive/`, `deliverables/`, backups, logs, caches, `node_modules/`, or build output.
...
git commit -m "Release linux-agent-env v5.0.1"
git tag v5.0.1
git push
...
V5 release focus: current WSL 2 + WSLg setup, normal LeanCTX/Orca onboarding, safer account creation, current installer precedence, and release-ready documentation hygiene.

## Release history

README only keeps the compact major-version history. See [CHANGELOG.md](CHANGELOG.md) for the fuller release notes.

- **v5.x** - Current major line. Targets current WSL 2/WSLg and upstream Agent releases, removes legacy compatibility paths, preserves the unattended `agent / cxthhhhh.com` bootstrap account, and keeps launcher/provider verification explicit.
- **v4.x** - Release-hardened Agent environment toolkit shaped by CXT/MeowLove real deployments and upstream issue discussions: version-consistent publishing, official-source recheck notes, launcher-safe token saver guidance, GUI/IDE/service discovery vs runtime-injection separation, and stricter release hygiene.
